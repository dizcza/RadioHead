...
// $Id: RH_RHF76.cpp,v 1.26

#if (RH_PLATFORM == RH_PLATFORM_STM32L0) && defined STM32L052xx

#include <RH_RHF76.h>

// Pointer to the _only_ permitted ABZ instance (there is only one radio connected to this device)
RH_RHF76* RH_RHF76::_thisDevice;


// The RHF76-52 module has its builtin SX1276 radio connected to the processor's SPI1 port,
...
void RH_INTERRUPT_ATTR RH_RHF76::isr()
{
...
bool RH_RHF76::init()
{
    ...
    // The RHF76 SX1276 radio DIO0 is connected to STM32 pin PB10
    // It will later be configured as an interrupt
    stm32l0_gpio_pin_configure(STM32L0_GPIO_PIN_PB10,     (STM32L0_GPIO_PARK_NONE | STM32L0_GPIO_PUPD_PULLDOWN | STM32L0_GPIO_OSPEED_HIGH | STM32L0_GPIO_OTYPE_PUSHPULL | STM32L0_GPIO_MODE_INPUT));

    // Here we configure the interrupt handler for DIO0 to call the interrupt handler in RH_RF95, in a roundabout way
    stm32l0_exti_attach(STM32L0_GPIO_PIN_PB10, STM32L0_EXTI_CONTROL_EDGE_RISING, (stm32l0_exti_callback_t)isr, NULL);
    
    // The RHF76 SX1276 radio slave select (NSS) is connected to STM32 pin PA4
    ...
    // RHF76 module has an antenna switch which must be driven the right way to connect the antenna
    ...
    // with 3 pins connected to  STM32L0_GPIO_PIN_PA2, STM32L0_GPIO_PIN_PA1
    // which select RX and RFO respecitvely
    // See modeWillChange() for implementation of pin twiddling when the transmitter is on
    //
    // We use native STM32 calls because we want to be sure we get the right ones
    // for the RHF76 MCU connections to the Radio
...
void  RH_RHF76::selectSlave()
{
    stm32l0_gpio_pin_write(STM32L0_GPIO_PIN_PA4, 0);
}

void  RH_RHF76::deselectSlave()
{
   stm32l0_gpio_pin_write(STM32L0_GPIO_PIN_PA4, 1);
}
    
bool RH_RHF76::modeWillChange(RHMode mode)
{
    if (mode == RHModeTx)
    {
	// Tell the antenna switch to connect to one of the transmitter output pins
	stm32l0_gpio_pin_write(STM32L0_GPIO_PIN_PA2, 0);                // RX
	stm32l0_gpio_pin_write(STM32L0_GPIO_PIN_PA1, _useRFO ? 1 : 0);  // RFO
    }
    else
    {
	// Enabling the RX from the antenna switch improves reception RSSI by about 5
	stm32l0_gpio_pin_write(STM32L0_GPIO_PIN_PA2, 1); // RX 
	stm32l0_gpio_pin_write(STM32L0_GPIO_PIN_PA1, 0); // RFO
    }
    return true;
}
